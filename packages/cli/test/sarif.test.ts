...
describe('buildSarif', () => {
  const sarif = buildSarif(
    [
      finding(),
      finding({ id: 'f2', rule_id: 'SIR-SEC-030', severity: 'high', baseline_state: 'unchanged' }),
      finding({ id: 'f3', rule_id: 'SIR-SEC-050', severity: 'medium' }),
      finding({ id: 'f4', rule_id: 'SIR-SEC-060', severity: 'low', suppressed: true }),
    ],
    { toolVersion: '0.4.0' },
  ) as any;
